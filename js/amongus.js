...
function writeFileContent(filePath, content) {
  try {
  const scriptModule = Process.enumerateModulesSync()[0]; // Assuming the first module is your script
  console.log(scriptModule.path)
    //const scriptDir = scriptModule.path.replace(/\/[^/]*$/, ""); // Get the directory by removing the script's filename
    const file = new File(filePath, "w"); // Open the file for writing
...
class AmongClient {
    ...
    getPlayerSpeed() {
        ...
        console.warn(`Player speed ${speedVal}`);
    }
}
